...
import { tanggalPanjangWIB } from "@/lib/logic/waktu"
...
export function BerandaGuru({ awal }: { awal: DataBerandaGuru | null }) {
  // Seluruh nilai awal datang dari server lewat prop `awal`; halaman tidak
  // mengambil apa pun lagi setelah dihidrasi.
  const kelasAwal = awal?.kelas ?? []
  const jenisAwal = bacaJenisPelanggaran(awal?.pengaturanJenis ?? null)

  const [currentUser, setCurrentUser] = useState<any>(awal?.user ?? null)
  // Data sudah pasti ada saat render pertama, jadi tidak ada lagi keadaan
  // "belum dimuat" yang perlu ditunggu.
  const hasLoadedUser = true
  const [teacherClasses, setTeacherClasses] = useState<any[]>(kelasAwal)
  const [materials, setMaterials] = useState<any[]>(awal?.materi ?? [])
  ...
  const [jenisPelanggaran] = useState<JenisPelanggaran[]>(jenisAwal)
  const [violationType, setViolationType] = useState(jenisAwal[0]?.nama ?? "")
  const [violationPoints, setViolationPoints] = useState(jenisAwal[0]?.poin ?? 0)
      ...
      <div className="rounded-3xl bg-gradient-to-br from-emerald-700 via-teal-700 to-slate-900 p-4 text-white shadow-lg shadow-emerald-700/20 relative overflow-hidden">
        <div className="flex items-start justify-between mb-2">
          ...
          <span className="text-xs text-emerald-200 font-medium">
            {tanggalPanjangWIB(new Date(), { weekday: 'long', day: 'numeric', month: 'short' })}
          </span>
        </div>
