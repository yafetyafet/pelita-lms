...
import { submitAttendance, submitCheckOut, type getTodayAttendance } from "@/app/actions/student"
import { jamLabelWIB } from "@/lib/logic/waktu"
...
export function IsiPresensi({ awal }: { awal: { geo: Geofence | null; presensi: Awaited<ReturnType<typeof getTodayAttendance>> } }) {
  ...
  // dan kini sudah ikut terkirim bersama HTML.
  const [geo] = useState<Geofence | null>(awal.geo)
  const [statusMessage, setStatusMessage] = useState<string>("Menunggu pendeteksian lokasi...")
  
  // Keadaan presensi hari ini, sudah diambil di server.
  const p0 = awal.presensi
  ...
  const [checkOutTimeStr, setCheckOutTimeStr] = useState<string | null>(
    p0?.checkOutTime ? jamLabelWIB(p0.checkOutTime) : null
  )
  const [isSubmitting, setIsSubmitting] = useState(false)

  // Hanya GPS yang masih perlu dijalankan di peramban; datanya sudah ada.
  useEffect(() => {
    detectLocation(awal.geo)
  ...
  const handlePresensi = async () => {
    ...
    if (!attended) {
      ...
      if (res.success) {
        const now = new Date()
        setAttended(true)
        setAttendedTime(jamLabelWIB(now))
      } else {
      ...
      if (res.success) {
        const now = new Date()
        setCheckedOut(true)
        setCheckOutTimeStr(jamLabelWIB(now))
      } else {
